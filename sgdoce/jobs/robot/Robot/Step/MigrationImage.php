...
/**
 * Classe da rotina de migração de imagens de artefatos avulsos
 *
 * @package     Robot
 * @category    Step
 * @name        MigrationImage
 ...
 */
class MigrationImage implements \Robot\StepInterface
{
    /**
     * @var
     */
    ...
    public function exec()
    {
        Debug::log( sprintf( '%s: %s', __METHOD__,
                'Executar migração das imagem de artefatos avulsos do sistema antigo para o novo...' ),
            Debug::INFO, Debug::STEP_IMAGE );

        Debug::log( sprintf('\Zend_Version::VERSION: %s', \Zend_Version::VERSION), Debug::WARN, Debug::STEP_IMAGE );

        try {
            $this->_service->runArtefatoWithoutImage();
        } catch (\Exception $e) {
            $this->_fail( $e );
        }
    }
