...
class Core_View_Helper_ActionButtonDropdown extends Zend_View_Helper_Abstract
{
     ...
     */
    public function actionButtonDropdown(
        array $arrItem,
        $strTitle = '',
        $strPosition = 'left',
        $strIcon = 'cog',
    ...
    )
    {
        ...
        if(count($arrItem)) {
            $strRetorno .= '
                <button class="btn btn-mini dropdown-toggle" data-toggle="dropdown" title="' . $strTitle . '">
                    <i class="icon-' . $strIcon . '"></i></button>
        ...
        if ($grouped) {
            $strRetorno = '<div class="btn-group pull-' . $strPosition . '">'.$strRetorno.'</div>';
        }
