 ...
 * @version     1.0.0
 */
class Artefato_ProcessoEletronicoController extends Artefato_ArtefatoController
{
    /**
     * @var string
     */
    ...
    public function formDataAction()
    {
        ...
        if(!$sqArtefato) {
            ...
            $this->view->pageTitle = "Alterar Processo";
            ...
            if (!$this->getService('AreaTrabalho')->canEditArtefact($dtoCheckEdit)) {
                $this->view->bloqueioArtefato = true;
            }
    ...
    public function saveAction()
    {
        ...
        $this->_redirect("/artefato/area-trabalho/index/tipoArtefato/"
            . \Core_Configuration::getSgdoceTipoArtefatoProcesso() . "/caixa/minhaCaixa");
    }
    ...
    public function checkProcessoCadastradoAction()
    {
        ...
        $status = true;
        try {
            $status = $this->getService()->checkProcessoCadastrado($dtoSearch);
        } catch (Exception $ex) {
            $msg = $ex->getMessage();
        }
        $this->_helper->json(array('msg' => $msg, 'status' => $status));
    }
