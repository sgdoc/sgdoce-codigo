...
class Artefato extends ArtefatoExtensao
{
    ...
    {
        return $this->_getRepository('app:HistoricoArtefato')->searchPageDto('listGridHistorico', $dto, TRUE);
    }

    public function listGridHistoricoFisico($dto)
    {
        return $this->_getRepository('app:HistoricoArtefato')->searchPageDto('listGridHistoricoFisico', $dto);
    }
...
//         $cpfCnpjPassaport = $this->retornaRegistro($dto);
        ...
        if( $stProcedencia && !$dto->getSqPessoaEncaminhado() ) {
            $sqPessoaEncaminhado = null;
        } else if( !$stProcedencia && !$dto->getSqPessoaEncaminhadoExterno() ){
            $sqPessoaEncaminhado = null;
        }
    ...
    /**
     *
     * @param mexed $search (\Core_Dto_Search , \Sgdoce\Model\Entity\Artefato)
     * @return boolean
     * @throws \InvalidArgumentException
     */
    public function isInconsistent ($search, $onlyImage = false, $onlyData = false)
    {
        return $this->_getRepository('app:Artefato')->isInconsistent($search, $onlyImage, $onlyData);
    }
    ...
    {
        return $this->_getRepository('app:Artefato')->isMigracao($dtoSearch);
    }
    ...
    public function hasArtefatoDuplicado( $dto, $count = true )
    {
        return $this->_getRepository()->findArtefatoDuplicado($dto, $count);
    }

    public function getArtefatoDuplicado( $dto, $count = false )
    {
        return $this->_getRepository()->findArtefatoDuplicado($dto, $count);
    }
    ...
    {
        return $this->_getRepository('app:TramiteArtefato')->hasTramiteEfetivo($dto);
    }
