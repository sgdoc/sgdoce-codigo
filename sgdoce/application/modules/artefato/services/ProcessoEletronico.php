 ...
 * @version  1.0.0
 */
class ProcessoEletronico extends Artefato {
    ...
    public function preSave($objEntity, $objSearchDto = null, $objArtProDto = null) {
        ...
        if( $objArtProDto->getCoAmbitoProcesso() == 'F' ) {
            $nuArtefatoValue = preg_replace('/[^a-zA-Z0-9]/', '', $nuArtefato);
            if( strlen($nuArtefatoValue) > 17 && !$configs['processo']['numberWithNupSiorg'] ) {
                throw new \Core_Exception_ServiceLayer( Core_Registry::getMessage()->translate('MN204') );
            }
            // Retirado numeração de processo com 21 dígitos para ambito federal até janeiro.
            if( in_array(strlen($nuArtefatoValue), array(17)) ) {
     ...
     * @param \Core_Dto_Search $dto
     */
    public function listGrid(\Core_Dto_Search $dto, $withTotalRecord = TRUE) {
        $result = $this->_getRepository('app:ArtefatoProcesso')->searchPageDto('listGridProcesso', $dto, $withTotalRecord);
        return $result;
    }
}
