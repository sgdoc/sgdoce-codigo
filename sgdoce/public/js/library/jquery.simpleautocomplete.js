        ...
        simpleAutoComplete: function( page, options, callback )
        {
            ...
            thisElement.keyup(function( ev )
            {
                ...
                if ( typeof(options.extraParamFromInput ) != "undefined") {
                    $.each(options.extraParamFromInput.split(','), function (i, value){
                        getOptions.extraParam = $( value ).val();
                        getOptions[value.replace('#', '')] = $(value).val();
                    });
                }
                ...
                if ((key.match(/[a-zA-Z0-9_\- ]/) || kc == 8 || kc == 46) && getOptions.query.length >= options.minLength)
                {
                    ...
                    self.searching = setTimeout(function() {
                        $.get(page, getOptions, function(r) {
                            ...
                            if (r == '' || r.length == 0) {
                                //caso o index '__NO_CLICK__' esteja setado na resposta da solictação
                                //não é substituido por este. Logo a mensagem pode ser personalizada
                                r = {__NO_CLICK__:"Sua pesquisa não retornou resultado."};
                            }

                            var js_counter = 0;
                            $.each(r, function(index, value) {
                                var id = 'autocomplete_'+index;
                                curr_li = $('<li>' + value + '</li>')
                                        .attr('id', id)
                                        .attr(attrCB, id)
                                        .data('id', index).appendTo(lista);
                                if (js_counter ==0)
                                {
