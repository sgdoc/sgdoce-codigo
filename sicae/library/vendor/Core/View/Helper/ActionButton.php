 ...
 * @todo       review
 */
class Core_View_Helper_ActionButton extends Core_View_Helper_Abstract
{
    /**
     ...
     * @param array $resource array com action/controller/module para verificaçao no acl para printar ou nao o botao
     */
    public function actionButton($title, $target, $params = array(), $icon = NULL, $label = NULL,
                                 $tagAttrs = array(), $targetType = 'js', array $resource=array())
    {
        //se tiver resource verifica a permissão
        if ($resource && !$this->_checkAcl($resource)) {
            return '';
        }
        ...
        return "<a class=\"$classesDefaults\" title=\"$title\" href=\"$target\" $attrs><span class=\"$icon\">$label</span> </a>\n";
    }
}