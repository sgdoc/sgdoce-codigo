...
class Core_Application extends Zend_Application
{
    ...
    protected function _loadCache($idCache)
    {
        ...
        // verifica cache salvo no backend
        $cacheOnBackend = $cache->getBackend()->test($idCache);

        if ($cacheOnBackend) {
            $dataCache        = $cache->load($idCache, true);
            $options          = (array) $dataCache['options'];
            $this->_filesRead = (array) $dataCache['filesRead'];

            if ($cache instanceof Zend_Cache_Frontend_File) {
                $files             = (array) $cache->getOption('master_files');
                $shouldMasterFiles = false;

                $filesRead      = $this->_filesRead;
                $filesIntersect = array_intersect($files, $filesRead);

                foreach ($filesIntersect as $file) {
                    if (false !== $key = array_search($file, $files)) {
                        unset($files[$key]);
                    }

                    if (false !== $key = array_search($file, $filesRead)) {
                        unset($filesRead[$key]);
                    }
                }

                $diff = $filesRead + $files;

                if (count($diff)) {
                    $cache->setOption('ignore_missing_master_files', true);
                    $masterFiles = $cache->getOption('master_files');
                    array_splice($masterFiles, count($masterFiles), 0, $diff);
                    $cache->setOption('master_files', $masterFiles);
                    return array();
                }
            }

            $cached = $cache->test($idCache);
    ...
    protected function _mergeFilesRecursive(array $options)
    {
        if (is_array($options['config'])) {
            ...
            $this->_filesRead[] = $options['config'];
