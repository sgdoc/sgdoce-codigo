...
class Core_Configuration extends Zend_Config
{
    ...
    public static function __callStatic($method, array $args)
    {
        ...
        if ('get' !== $command) {
            throw new BadMethodCallException(
                sprintf('Método "%s::%s()" não começa com "get".', get_class($object), $method)
            );
        }
        ...
        if (!isset($object->$const)) {
            throw new BadMethodCallException(
                sprintf('Constante de sicae.configuracao "%s" não registrada.', $const)
            );
        }
    ...
    protected function initConstants()
    {
        ...
        $query->useResultCache(TRUE, 3600, 'SICAe_Main_Application_Constant');
