...
class Core_Model_OWM_Repository implements ObjectRepository
{
    ...
    public function _triggerMethods($method, $args)
    {
        ...
        catch (SoapFault $sExcp)
        {
            throw new Exception($method, 0, $sExcp);
        }

        $result = $this->_substituirCaracterEspecial($result);

        if ( ! $xml = simplexml_load_string($result) ) {
            throw new Exception($this->_handler->getLastResponse());
        }
     ...
     */
    private function _substituirCaracterEspecial($value)
    {
